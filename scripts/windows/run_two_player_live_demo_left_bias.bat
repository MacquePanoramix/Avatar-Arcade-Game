...
start "OpenPose Live (Two Player Left-Bias)" cmd /k "cd /d \"%OPENPOSE_DIR%\\bin\" && OpenPoseDemo.exe --model_folder \"%OPENPOSE_MODEL_DIR%\" --write_json \"%LIVE_SESSION_DIR%\" --display 1 --render_pose 1"

start "Classifier Live (Two Player Left-Bias)" cmd /k "cd /d \"%REPO_DIR%\" && python -m src.inference.live_openpose_debug --json-dir \"%LIVE_SESSION_DIR%\" --model-path \"%MODEL_PATH%\" --output-latest-json \"%JSON_OUT%\" --output-jsonl \"%JSONL_OUT%\" --log-csv \"%CSV_OUT%\" --tracking-mode two_player_left_right --side-split-x 380 --overlay-mode both --live-source-fps 12 --require-motion-for-nonidle --accept-threshold 0.55 --margin-threshold 0.02 --trigger-streak 1 --trigger-cooldown-frames 5 --release-idle-frames 1 --motion-on-min-consecutive 1 --active-span-min-frames 2"
